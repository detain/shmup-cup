...
|---|---|---|
| [`client/preview-build.md`](client/preview-build.md) | Testers / owner | The current game build (free flight — steering the KESTREL with the remote, keyboard or gamepad while its gun fires on its own; the scrolling test stage with its enemies, their bullets and shooting them down, the fully powered ship with Options, laser and Force Field, power capsules and equipping them with OK, losing the ship — the freeze, the blinking comeback, what a loss costs, GAME OVER — and the score and `HI`, the boss range with its WARNING sign and the first boss, the explosions, sparks, screen shake, flashes and score numbers, the sound effects and the music — the stage theme, the WARNING siren, the boss theme, the stage-clear jingle —, sprite showcase, calibration screen and effects gallery in a browser on request): opening it on the TV, in a browser and in Electron, how the ship should respond, what to check on the monitors, the loading bar and the boot error screen, what a correct picture and sound look like, what to report, troubleshooting |
