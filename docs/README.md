...
|---|---|---|
| [`client/preview-build.md`](client/preview-build.md) | Testers / owner | The current game build (free flight — steering the KESTREL with the remote, keyboard or gamepad; sprite showcase and calibration screen on request): opening it on the TV, in a browser and in Electron, how the ship should respond, what to check on the monitors, the loading bar and the boot error screen, what a correct picture looks like, what to report, troubleshooting |
...
| [`../content/README.md`](../content/README.md) | Content formats for authors (player ships, weapons, enemies, stages, input profiles), one README per folder |
