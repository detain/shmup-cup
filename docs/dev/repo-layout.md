...
├── scripts/                repo-level Node scripts: clean.mjs, generate-assets.mjs (pnpm assets) + assets/ (PNG encoder, sprite sources, procedural generators, packer, font, the `@flash` and — M2-06, coop.mjs — player 2's `@p2` palette-swap siblings; the raster-bands generator — M2-08; the bosses generator — turret heading frames, orb, raid hull, captain shell — M2-09; the zone generators brine.mjs / dune.mjs and the zone tilesets of terrain.mjs — M2-11; magma.mjs / tempest.mjs — M2-12), gen-trig-tables.mjs, audio-preview.mjs (pnpm audio:preview → WAV files), golden-update.mjs (pnpm golden:update), content/tiled-import.mjs (pnpm content:tiled: a Tiled map → stage JSON — M2-07)
