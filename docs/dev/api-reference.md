...
| `SfxPriority` | const + type | Priority hints in an `Sfx` event's `param` (M1-13): `Default 0` (the cue's own priority), `Low 1`, `Normal 2`, `High 3` (a boss's final blast), `Critical 4` (never stolen — the WARNING siren); the M1-15 mixer maps them to its tiers |
...
since M1-14) and — only for the default `input-profiles` content owner — `@shmup/input-web`;
the input and audio adapters come in through core interfaces.
...
| `ShellOptions` | `boot` | `canvas`, `win`, `contentFiles`, `assets`, `input`, `audio`, `platform: (renderer) => Platform` (called after content validation — the apps apply the input profiles there), `gameConfig?`, `scene?` (`'flight'`), `audioUnlock?` (`'gesture'` \| `'immediate'`), `preferWebGLVersion?` (1), `contentOwners?` (merged over `DEFAULT_CONTENT_OWNERS`; an `fx` owner here replaces the shell's own and leaves the particles without presets), `effects?` (`Partial<EffectSettings>` for the renderer, M1-14), `createImage?`, `overlay?` (`null` disables it) |
| `Shell` | `boot` | `game`, `platform`, `renderer`, `atlas`, `events` (dispatcher), `content`, `scene`, `flight` (the `FlightScene` or `null`), `showcase` (or `null`), `fxGallery` (the `FxGallery` or `null`, M1-14), `fx` (the `content/fx/` presets handed to the renderer, M1-14), `stop()` (idempotent; releases loop, listeners, input, renderer, atlas, audio). Boot also seeds the particles with `(gameConfig.seed ^ 0x2545f491) >>> 0` and, in free flight, connects the World's events (`connectFxEvents`) |
...
| `DEFAULT_CONTENT_OWNERS` | `loader` | Frozen owners of today's foreign kinds: `input-profiles` → input-web `loadInputProfiles`, `fx` → render-pixi `loadFxContent` (M1-14) — issues only. An app entry of the same kind replaces it |
