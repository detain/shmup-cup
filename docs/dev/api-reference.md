...
| `ContentDb` | interface | `sprites` (content sprite names + `extraSprites`), `scripts` (`StringTable`), `ships`, `weapons`, `weaponPresets`, `enemies`, `paths`, `stages`, `tilesets`, each with an id → position `…Index` map (`shipIndex`, `weaponIndex`, `weaponPresetIndex`, `enemyIndex`, `pathIndex`, `stageIndex`, `tilesetIndex`), and `difficulty` (M2-01: the frozen `DifficultyTable` of a `rules` file's `difficulty` section, `null` when no file has one — sessions then use `core/config` `DEFAULT_DIFFICULTY_TABLE`) |
...
| `CONTENT_KINDS`, `ContentKind`, `isContentKind(kind)` | const/type/function | `player`, `weapons`, `enemies`, `paths`, `stage`, `tileset`, `rules` (M2-01); other kinds come back in `foreign` |
...
ignored. Guide: [difficulty-and-rank.md](difficulty-and-rank.md#the-rules-kind-coredata).
...
| `EnemyHost` | interface | What the system reads from its World: `tick`, `camera`, `players`, `ship`, `terrain`, `content`, `rng`, `events`, `debugFlags`, `bullets` (M1-09: fire primitives; lasers detach when their enemy goes), `rank?` (M2-01: the session's rank for revenge bullets; absent = 0) |
...
tick (M1-12); extends and the continue digit (M2-01). 1UP items arrive with Direct mode (M2-05),
...
Still planned inside the partial modules: `config` — display options (M2-08 / M2-16), the chosen
...
leaving (M2-06); `scoring` — 1UP items (Direct mode, M2-05); `fx` — authentic slowdown (M3-02); `collision` — circle chains
(M2-02), destructible tiles (M2-07); `debug` (implemented) — the device info in the overlay
...
`patterns` — `compilePattern` (M2-02); `enemies` — the Option Hunter (M2-04); `behaviors` — the behaviours of the M2 zones (M2-11 … M2-14);
...
points, the pattern DSL's bullets (M2-02), graze (P2); `rank` (implemented) — the loop / stage
