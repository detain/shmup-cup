...
| Export | Kind | Summary |
|---|---|---|
| `createListMenu(items, { focus?, disabledMask?, wrap? })` | function | → `ListMenu` (load time); `items` = 1–31 `MenuItemSpec`s (a label = an action, `{ label, slider }`, `{ label, toggle }`); focus = the first enabled item at or after `focus` (default 0); `wrap` default `true`; throws `RangeError` for 0 or > 31 items |
...
| `MenuItem`, `MenuItemKind`, `MenuItemSpec`, `ListMenuOptions` | interface, const + type, type, interface | `{ label, kind, slider, toggle }`; `Action 0`, `Slider 1`, `Toggle 2`; the `createListMenu` input |
...
| `drawMenu(list, menu, stringBase, layout)` | function | → the y below the last row: labels (focus colour + `→` cursor, disabled dimmed), a slider's 50-px bar and value, a toggle's `ON` / `OFF`; uses string slots `stringBase … + menuStringSlots(menu) − 1`, written only when changed. `layout` must be a constant (a literal per call allocates) |
| `MenuLayout` | interface | `x`, `y`, `lineHeight?` (12), `align?` (`TextAlign`, Left), `cursorX?` (`x − 10`), `valueX?` (`x + 80`) |
| `menuStringSlots(menu)`, `CONFIRM_STRING_SLOTS` | function, const | `items + 3` (items, `ON`, `OFF`, cursor); `4` (question, YES, NO, cursor) |
...
The scene stack and the M1 scene set (M1-16). Guide: [scenes-and-ui.md](scenes-and-ui.md).
...
| `SceneFlowHost` | interface | `config`, `content`, `events`, `exit` (`platform.exit` or `null` — no EXIT item, Back on the title only backs out), `createWorld()` (a fresh World pushing into `events`) |
| `SceneFlow` | interface | `stack`, the seven scenes (`boot`, `title`, `game`, `pause`, `stageClear`, `gameOver`, `confirm`), `inputContext`, `world` (the game scene's), `menuInput` (every player merged, reused), `hiScore` (the session's best), `view` (`SceneFlowView`), `tick(input)`, `updateFrame()` (once per displayed frame: World view + HUD while the game is visible, the top dim, the UI list rebuilt only when the visible set or a `uiRevision` changed), `setBootProgress(fraction, label?)`, `finishBoot()`, `onResume()` (pause menu over a running game), `setHiScore(value)` (raise only; floored, capped at `MAX_SCORE`) |
...
| `TitleScene` | class | `menu` (START / OPTIONS disabled until M1-17 / EXIT only with `platform.exit`), `phase` (0 `PRESS OK`, 1 menu), `menuOpen`; the logo, a 32-tick blink, the session hi-score, the title music; Back → the exit confirmation, or (no exit) back to `PRESS OK` |
| `GameScene` | class | `world`, `hud`, `hudList` (64 commands), `starts`, `restart()`; `inputContext` `'game'`; Pause / Back (any player) → pause menu; opens stage clear `STAGE_CLEAR_DELAY_TICKS` / game over `GAME_OVER_DELAY_TICKS` World ticks after the status changed; draws the boss WARNING band |
| `PauseScene` | class | Overlay, dim `PAUSE_DIM`: `menu` (RESUME / OPTIONS disabled / RETRY STAGE / QUIT TO TITLE); Pause / Back / RESUME resume, RETRY restarts without a confirmation, QUIT asks |
| `StageClearScene` | class | Overlay (dim 0.25): `phase`, `ticks`; tally `STAGE_CLEAR_TALLY_TICKS` → `TO BE CONTINUED` `STAGE_CLEAR_CONTINUED_TICKS` → title; OK skips |
| `GameOverScene` | class | Overlay (dim 0.35): `ticks`; OK / Back after `GAME_OVER_LOCK_TICKS`, or `GAME_OVER_TIMEOUT_TICKS` → title |
...
| `ConfirmPurpose`, `TitleItem`, `PauseItem` | const + type, const, const | `Exit 0`, `QuitToTitle 1`; `Start 0`, `Options 1`, `Exit 2`; `Resume 0`, `Options 1`, `Retry 2`, `Quit 3` |
...
`shields` in M1-11, `scoring` and `fx` in M1-12, `bosses` in M1-13, and `ui` and `scenes` in
M1-16.
...
(M2-01), `insertHiScore` (M1-17 / M2-15); `fx` — authentic slowdown (M3-02); `collision` — circle chains
...
the Option Hunter (M2-04); `behaviors` — the zone behaviours and HALCYON BULWARK's (M1-18);
...
Reduce (M2-04), the Arm tiers with repair (M2-05); `scenes` — the Options overlay (M1-17),
attract mode, mode / ship / weapon select, the zone map, name entry, the hi-score table, ending
and credits (M2); `ui` — the key-rebind prompt and the name entry (M2-15 / M2-16), the boss HP
...
`web-audio` stays `partial` (the Options sliders drive `setBusVolume` in M1-17); `synth`, `sfx`,
...
| `SCENE_ATTRIBUTE` | `boot` | `'data-shmup-scene'` — the scene flow's top scene id (`title`, `game`, `pause`, `confirm`, …) or the dev scene's name (`flight`, `showcase`, …) on the game canvas (M1-16; tests, the TV's remote inspector) |
...
| `generate()` per module; `BULLET_COLORS`, `METER_LABELS`, `TERRAIN_TILES`, `TILE_SIZE` (8), `STAR_TILE_SIZE` (128) | `procedural/*.mjs` | The generators (`bullets`, `explosions`, `hud`, `items`, `lasers` — M1-09: `lasers/beam-{pink,red,purple}`, 8 frames of 4×8, frame `k` a band `k + 1` px tall; `BEAM_WIDTH`, `BEAM_HEIGHT`, `bandRows` — `particles`, `shields`, `starfield`, `terrain`, `ui` — M1-16: `ui/logo`, 165×27, from original 5×7 block letters; `LOGO_TEXT` `'SHMUP CUP'`, `LOGO_SCALE` 3) and their data |
