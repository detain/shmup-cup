...
| `ReplayHeader`, `ReplayHeaderOptions` | interfaces | `formatVersion`, `buildId`, `seed`, `config` (the whole resolved `GameConfig`), `stageId`, `checkpoint`, `loadout`, `assisted` (god mode for the whole run); the options above |
...
| `encodeReplay(replay)`, `decodeReplay(data)` | functions | ↔ `ReplayJson { kind: 'replay', header, ticks, hashInterval, inputs (base64 RLE per player), hashes, finalHash }`; `decodeReplay` validates everything and throws `RangeError` naming the problem (extra top-level fields ignored, unknown config keys dropped, missing ones defaulted) |
...
M1-09, `weapons` and `options` in M1-10, `powerups` and `shields` in M1-11, `scoring` and `fx` in
