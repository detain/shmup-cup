...
| `pnpm store:assets` | M2-18: draws the icon set and the store-listing placeholders from the placeholder art (`scripts/store-assets.mjs`): the committed icons `apps/tizen/public/icon.png` (512 × 423) and `apps/electron/build/icon.png` (512 × 512), and the git-ignored `assets/generated/store/` (icon, four 1920 × 1080 placeholder screenshots, `listing.json`). `--check` exits 1 when a committed icon differs from a fresh rendering (`pnpm test` checks the same); `--out DIR` writes the store folder elsewhere. Run it and commit the icons after changing the logo, the ships or the script — see [release-hardening.md](release-hardening.md#icons-and-store-listing-placeholders) |
