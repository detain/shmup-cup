...
`music: { stage, boss }` (`MUSIC_CUES` names; since M2-14 optionally `ending` and `credits` — a final zone's themes for the ending screen and the credits, prepared with its set), `length` (camera-x length in pixels), `camera`
...
# → http://localhost:5173/?stage=zone-h       IRON CITADEL alone — its piston hall and the parade (M2-14)
...
M2-14 replaced the last two, the finales: `zone-h.stage.json` (IRON CITADEL, 9,600 px,
`terrain-citadel`, eleven moving-block **pistons** — `block` events of the tileset's `solid` tile
swinging ±22 px out of a flat floor and ceiling —, four `boss` events for the parade's captains, a
1.3 px/tick core run with a `haze`) and `zone-i.stage.json` (ABYSSAL THRONE, 9,600 px,
`terrain-abyss`, a trench of floor and ceiling, open water from the calm on for the ARK's raid, two
`wave` rasters, a 1.3 px/tick undertow). Both name the final zones' extra music cues,
`"music": { "stage": "Stage", "boss": "FinalBoss", "ending": "Ending", "credits": "Credits" }` —
content only, on the runner as it is ([zones-h-and-i.md](zones-h-and-i.md)).
