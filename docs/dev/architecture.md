...
`behaviors` (partial: the M1 enemy and boss rosters, `pattern.loop`, `hunter.option`, `cube.pincer`, the six stage gimmicks of M2-07, the captains and raid turrets of M2-09, zones B and C's `rocket.homing`, `worm.burst`, `boss.maw` and `boss.widow` of M2-11, zones D and E's `rear.swoop`, `boss.bastion` and `boss.steed` of M2-12), `bosses` (implemented with M2-09: the P0
