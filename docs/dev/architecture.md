...
`behaviors` (partial: the M1 enemy and boss rosters, `pattern.loop`, `hunter.option`, `cube.pincer`, the six stage gimmicks of M2-07, the captains and raid turrets of M2-09, zones B and C's `rocket.homing`, `worm.burst`, `boss.maw` and `boss.widow` of M2-11), `bosses` (implemented with M2-09: the P0
...
(M2-03; its OPTION row M2-04), the ship select (M2-05), 1 PLAYER / 2 PLAYERS and the co-op rules (M2-06), campaign runs — the run state carried between zone Worlds, the zone tally, the zone map, the ending hook, hidden bonus stages, practice plumbing (M2-10) — the other M2 screens later);
...
| A zone (a stage with its roster and boss) | JSON under `content/stages/`, `content/enemies/`, `content/paths/`; `pnpm content:check`; a playtest run with the 4-way bot (`test/playtest/`) and its design-rule checks — [zone-a-and-playtest.md](zone-a-and-playtest.md#extending-it); the full recipe of M2-11 (roster, songs, tileset, generator, checks) is in [zones-b-and-c.md](zones-b-and-c.md#building-the-next-zone-m2-12--m2-14) |
