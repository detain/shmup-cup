...
| `bonus` | M2-10 (hashed): the stage's hidden bonus-stage entrances (`core/stage` `BonusEntrances`) — armed by `bonus` events, tested in phase 3, re-armed by a checkpoint restart; `bonus.entered` tells the scene flow to warp into the bonus stage; none in free flight ([campaign-and-bonus-stages.md](campaign-and-bonus-stages.md#the-world-side-corestage-bonusts-worldbonus)) |
...
`PlayerState` is `'entering' | 'alive' | 'dying' | 'dead' | 'respawning' | 'leaving'`
(`PLAYER_STATES` gives the hash code order; `leaving` — code 5 — was appended by M2-10). `createPlayer(slot, lives)` makes a ship `dead` and inactive;
...
**The fly-out (M2-10).** Once the status is `stageClear`, phase 2 calls `flyOutPlayer` for every
active `alive` ship (from the tick after the clear; a ship still flying in follows after its
fly-in): the ship turns `leaving` — no control, no bank, no invulnerability blink — and
`updatePlayer` rides the camera scroll plus `LEAVE_ACCELERATION` (0.125) px/tick per tick up to
`LEAVE_MAX_SPEED` (8), stopping at `camera.x + LEAVE_END_X` (the playfield width + 32, out of
view). Only `alive` ships are hit, so a leaving ship is safe; the weapon system does not fire for
it but its Options trail it out ([campaign-and-bonus-stages.md](campaign-and-bonus-stages.md#launch-intro-and-fly-out)).
...
15. the campaign pieces (M2-10, `mixBonus`): the enemy totals (`stats.spawned`, `killed`,
    `groundSpawned`, `groundKilled`), then the bonus entrances — their `count`, per entrance its
    armed flag (and, while armed, the ground counters it started from), then `entered`,
    `enteredTick` and `locked`. (The `leaving` player state is a code of step 6.)
...
- **M2-10** (done) — `world.bonus` (the hidden bonus-stage entrances, phase 3, hashed with the
  enemy system's new `stats`), the stage-clear fly-out in phase 2 (`PlayerState` `leaving`), the
  campaign's `rankInputs.stage`; every zone of a run is a fresh World the scene flow builds with
  the players carried in ([campaign-and-bonus-stages.md](campaign-and-bonus-stages.md)).
