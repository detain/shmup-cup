...
11. the power-ups (M1-11): per player the meter `cursor`, the pending Mega Crash flag and the
    ...
    `spin` and every pod slot's hits, angle, i-frames and hit tick), then `dropsTaken` (the
