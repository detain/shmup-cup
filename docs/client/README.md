...
- **Options** — a page of its own once there are more options: rebinding buttons, the game
  ...
  SHAKE, FLASHES and HITBOX) is described in
...
FIT or STRETCH), whether the picture shakes (**SHAKE**), gentler flashes (**FLASHES**: REDUCED) and
a marker on the ship's weak spot (**HITBOX**); the game remembers them, and it remembers your **high scores** too — the title's `HI` is the best score kept on this
...
([preview-build.md](preview-build.md#the-raster-range-browser-only)). Hits *feel*
...
This is version **0.1.0**, the end of the first milestone, plus the first eight steps of the second
...
effects of the browser's Raster Range). For the on-device checks there is a
