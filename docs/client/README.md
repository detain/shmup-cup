...
- **Options** — audio volumes, display (scale modes, screen shake, flashing), controls,
  difficulty (the options screen arrives with the next build; OPTIONS is greyed out until
  then).
...
OPTIONS (greyed out for now) and, on the TV, **EXIT**. START begins a game under the new **HUD**:
your score, the session's best (`HI`) and the second player's slot along the top; your spare
...
bottom. **Back** (or Play/Pause) now **pauses**: RESUME, RETRY STAGE or QUIT TO TITLE. Losing the
...
shows the session's best ([preview-build.md](preview-build.md#lives-losing-your-ship-and-the-score)).
...
keyboard and gamepad are in [controls.md](controls.md). Next come the options screen (volumes,
the control profile) and saved high scores.
