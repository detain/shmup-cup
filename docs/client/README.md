...
STEED sending out little homing seahorses from its chest). Zones F–I are short stand-ins for now
(zone A's enemies and known bosses), so every route can be flown to the end; the real zones come
in the next builds. In a browser `?stage=bonus-range` shows all
...
This is version **0.1.0**, the end of the first milestone, plus the first twelve steps of the second
...
enemies from behind, the storm —, with their bosses CINDER BASTION and SQUALL STEED). For the
