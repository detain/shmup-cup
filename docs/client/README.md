...
when destroyed. In a browser, `?loadout=full` starts fully powered — a piercing laser, missiles
that slide along the ground and four Options that follow the ship and copy its fire
...
ship just disappears). If a build is broken it shows a readable error screen instead of a black
one. The button layouts for the TV remote, keyboard and gamepad are in
[controls.md](controls.md). Next come the power-ups: capsules and the power meter.
