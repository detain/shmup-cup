...
H or I an **ending** card names the ending your route and run earned. Zones B–I are short
stand-ins for now (zone A's enemies and known bosses), so every route can be flown to the end;
the real zones come in the next builds. In a browser `?stage=bonus-range` shows the **hidden bonus
stages** of the later zones: three secret entrances — a marked gap, all ground targets destroyed,
a score digit — into a vault of 1,000-point capsules and an extra ship; a loss there locks you out
([preview-build.md](preview-build.md#the-zone-map-a-run-through-nine-zones),
[preview-build.md](preview-build.md#hidden-bonus-stages-browser-only-for-now)).
...
This is version **0.1.0**, the end of the first milestone, plus the first ten steps of the second
...
result, the fly-out, the ZONE MAP, stand-in zones B–I and a placeholder ending — and the hidden
bonus stages of the browser's Bonus Range). For the on-device checks there is a
