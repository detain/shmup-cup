...
- **Options** — a page of its own once there are more options: display (scale modes, screen
  ...
  penalty). Today's Options screen (volumes, the control profile and the bullet colours) is
  described in [preview-build.md](preview-build.md#the-options-screen).
...
This is version **0.1.0**, the end of the first milestone, plus the first two steps of the second
(difficulties, extra ships and continues; colour-blind bullet colours and points for bullets
cancelled by a destroyed boss or a Mega Crash). For the on-device checks there is a
