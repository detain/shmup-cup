...
| Page | Contents |
|---|---|
| [`preview-build.md`](preview-build.md) | The current game build: the **calibration screen** — how to open it on the TV, in a browser and on the desktop, what a correct picture looks like, what to report, troubleshooting |
| [`controls.md`](controls.md) | Default controls for the Samsung Smart Remote (primary), gamepads and keyboard, and what each action does |
...
([preview-build.md](preview-build.md)). Behind the scenes the engine foundations, the
first game data (the KESTREL ship and its weapons) and the placeholder graphics (a sprite
sheet and pixel font built into the app) are in place; nothing new is visible yet.
