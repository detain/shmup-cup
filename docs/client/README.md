...
| Page | Contents |
|---|---|
| [`preview-build.md`](preview-build.md) | The current game build: the loading bar and **free flight** (flying the KESTREL around an empty starfield with the remote, keyboard or gamepad), plus the scrolling **Test Range** stage with its enemies and their bullets, the sprite showcase and the calibration screen in a browser — how to open it on the TV, in a browser and on the desktop, how the ship should respond, what a correct picture looks like, which enemies shoot what, the start-up error screen, what to report, troubleshooting |
...
three-way fans and rings, all dodgeable with the four arrow directions. The enemies cannot be
shot yet; there are no weapons or sound yet, and touching the rock, an enemy or a bullet does
nothing yet (a bullet that touches the ship just disappears). If a build is broken it shows a
readable error screen instead of a black one. The button layouts for the TV remote, keyboard
and gamepad are in [controls.md](controls.md). Next come the ship's weapons.
