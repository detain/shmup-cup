...
caves, speeding up and slowing down, with star layers moving behind — and the **first
enemies** come at you: weaving pods, looping spinners, saucers, ground turrets, walkers,
hatches releasing small fighters. They cannot be shot yet and do not shoot back; there are no
weapons or sound yet, and touching the rock or an enemy does nothing yet. If a build is broken
it shows a readable error screen instead of a black one. The button layouts for the TV remote,
keyboard and gamepad are in [controls.md](controls.md). Next come weapons, then enemy bullets.
