...
fly into the gap between two reef blocks at the top of the screen, about 2:10 into the zone. Zones
D–I are short stand-ins for now (zone A's enemies and known bosses), so every route can be flown
to the end; the real zones come in the next builds. In a browser `?stage=bonus-range` shows all
...
This is version **0.1.0**, the end of the first milestone, plus the first eleven steps of the second
...
bonus stage PEARL GROTTO, and C, DUNE EXPANSE, with their bosses GALVANIC MAW and SANDGRAVE WIDOW). For the on-device checks there is a
