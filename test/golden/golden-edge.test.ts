...
describe.skipIf(updating)('golden replays — file guards', () => {
  it('has one file per scenario and no file without one', () => {
    ...
    // M2-09 (captains, the raid, the twins) and M2-10 (the bonus range and its vault); zones B and
    // C and zone B's bonus stage (M2-11); zones D and E (M2-12).
    for (const name of names) {
      expect(name).toMatch(
        /^(zone-[a-e]|gimmick-range|raster-range|captain-range|raid-range|twin-range|bonus-range|bonus-vault|brine-grotto)-[a-z0-9-]+$/,
      );
    }
