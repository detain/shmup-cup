/**
 * Allocation guard of `boss.widow` (plan M2-11; definition of done: zero allocations per
 * tick), in its own file so the worker's V8 type feedback comes only from this world: SANDGRAVE WIDOW in
 * its last phase in free flight, scuttling inside its box, spitting spreads from its head and spinning silk lines, the weaving, fully powered KESTREL's Laser and Options
...
/**
 * The KESTREL, Type A and zone c's roster, the boss's phases without minion launches.
 *
 * @returns The DB.
 */
function db(): ContentDb {
  const enemies = shipped('enemies/zone-c.enemies.json');
  const data = enemies.data as {
    enemies: { id: string; boss?: { phases: { params?: Record<string, number> }[] } }[];
  };
  for (const phase of data.enemies.find((e) => e.id === 'sandgrave-widow')?.boss?.phases ?? []) {
...
describe('core/behaviors boss.widow allocation (M2-11)', () => {
  it('allocates nothing per tick in SANDGRAVE WIDOW’s last phase', () => {
    const content = db();
    const w = createWorld(resolveGameConfig({ loadout: 'full', seed: 9 }), content);
    w.debugFlags.godMode = true;
    const input = createInputSnapshot();
    const boss = w.bosses.boss;
    expect(w.bosses.startBoss(content.enemyIndex.get('sandgrave-widow') ?? -1)).toBe(true);
    ...
    // Both fangs, then the head below 24 of 56: its last phase.
    for (const name of ['fang-top', 'fang-bottom']) {
      w.bosses.damagePart(
