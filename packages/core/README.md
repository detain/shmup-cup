...
| `createSceneFlow`, `SceneFlow`, `SceneStack`, `createSceneStack`, `Scene`, `SceneId`, `BootScene`, `TitleScene`, `GameScene`, `PauseScene`, `OptionsScene`, `StageClearScene`, `GameOverScene`, `ConfirmDialog`, `InputProfileSetup`, `mergeMenuInput`, … | `scenes` (partial) | The scene stack (depth 8, transitions deferred to the end of the tick) and the M1 flow (M1-16): boot → title (`PRESS OK`, START / OPTIONS / EXIT) → game (owns the World, HUD, WARNING band) ⇄ pause (RESUME / OPTIONS / RETRY STAGE / QUIT TO TITLE) → stage clear (tally, `TO BE CONTINUED`) / game over → title; the YES / NO dialog (Tizen exit confirmation: `platform.exit()` only after YES); the **Options** overlay (M1-17: MASTER / MUSIC / SFX sliders and CONTROLS applied live through `UserOption` events, BACK saves); finished games recorded in the save's hi-score table (`NEW HI-SCORE`), the title's HI from the saved best; Back walks the stack; the frame's world / HUD / UI list / dim (attract, select, map, name entry, more options: M2) |
...
| `hashWorld`, `createDebugFlags`, `DebugFlags`, `skipToBoss`, `BOSS_SKIP_LEAD` | `debug` (partial) | FNV-1a 32 state hash over tick, RNG states, camera, the stage runner's state, status, hit-stop, rank, players, every pool's live slots (the enemy bullets and lasers and the player shots among them), the enemies, the formation table, the player weapons' loadouts, Option trails, timers and cooldown tables, the power meters, pending Mega Crashes, shields and taken drops, the shake / flash timers, scores and credit counters — not the session hi-score — and the boss, its parts and the WARNING (M1-13) (golden replays); debug switches; the stage skip to 96 px before the first `warning` / `boss` event (`GameConfig.stageSkip`, M1-18) (controls: M1-19) |
