...
| `createWorld`, `WorldOptions`, `stepWorld`, `World`, `WORLD_PHASES`, `WorldPhase`, `syncWorldView`, `PoolRegistry`, `resolveWorldStage`, `ENGINE_SPRITES`, `DEATH_HIT_STOP_TICKS`, `DEATH_SHAKE_TICKS`, `DEATH_MUSIC_DUCK_TICKS` | `world` | The session state (tick, RNG streams, events, players, camera, status, hit-stop, the fx timers, debug flags, pools, the stage runner / terrain / parallax of `config.stage`, the enemy system, the rank, the bullet system, the weapon system, the power-up system and the scoring system, `view`) and the fixed 9-phase tick pipeline of plan §3.2; hit-stop skips phases 2–8 for exactly the requested ticks; the death sequence (phase 7), respawns with the `arcade` checkpoint restart and game over (phase 2) — M1-12; zero allocation per tick |
