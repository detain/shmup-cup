...
| `tracker` | implemented | The optional tracker-music path (M3-03): `detectAudioCapabilities` (AudioWorklet + WebAssembly), the `TrackerBackend` port a build plugs libopenmpt into, and `chooseMusicPath` — `tracker` → `file` → `song` → `none`, where `tracker` needs a module in the content, both capabilities **and** a backend. **No backend ships**: `chiptune3`'s worklet is ≈ 518 KB gzipped against a 512 KB bundle budget (`trackerFitsBundle()` → `false`), so it could only ever be a separately loaded file. A track's optional `module` sits *alongside* its song or file, never instead of it |
