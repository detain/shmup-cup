...
// connectStagePreparation (M2-10); the sound test's SoundTest events play a music-library track
// through connectSoundTest (M2-15)
```
...
`applyAudioOptions` and the saved input profile through `inputProfiles.apply(id, 'save')`, then —
M2-16 — the player's rebinding, SOCD and debounce through `inputProfiles.customize`) → game
(the scene flow gets the `SaveStore`, the profile choices, — M2-15 — the music library's titles
for the sound test and — M2-16 — the rebind screen's host side, `createShellControls`, when the app
offers `customize` / `rebindable` and the adapter the capture) → audio (M1-15: the engine renders the SFX
...
`connectStagePreparation` — and, since M2-15, `SoundTest` — `connectSoundTest`; since M2-16 the
`InputSettings` option re-applies the save's input settings through the app) → lifecycle (suspend; window `blur` clears held input — M1-17) /
...
scene — `title`, `difficulty`, `weaponSelect`, `autoOrder`, `game`, `pause`, `options`, `controls`, `display`, `gameOptions`, `rebind`, `inputTest`, `continue`, `confirm`, `stageClear`, `map`, `ending`, … — or the dev scene's name) and, once
...
|---|---|---|
| `boot` | implemented | `bootShell()`, `sceneFromSearch()`, `webGLVersionFromSearch()` (M3-02c — `?gl=1` / `?gl=2`, the review's F8 A/B switch; WebGL1 stays the shipped default), `ShellBootError`; `DEFAULT_STAGE_ID` / `defaultStageId(files)` — the stage the apps' scene flow plays, zone A (M1-18 — [`docs/dev/zone-a-and-playtest.md`](../../docs/dev/zone-a-and-playtest.md#the-game-plays-zone-a)); owns the audio engine (`Shell.audioEngine`, M1-15); reads the save before the title and exposes it (`Shell.loadedSave`, `Shell.save`), applies the saved volumes, input profile (`ShellOptions.inputProfiles`) and display options (`applyDisplayOptions`: the bullet palette — M2-02 —, the scale mode, shake, flashing and hitbox markers — M2-08; `ShellOptions.effects` wins), switches render interpolation from the refresh probe (`ShellOptions.interpolation`, `Shell.refresh` — M2-08) and the loop's vsync lock (`ShellOptions.framePacing` `'auto' | 'lock' | 'free'`, `game.setVsyncLock` — M3-02b), times the boot (`ShellOptions.now`, `Shell.bootTiming`, `BOOT_MS_ATTRIBUTE`), warms the renderer up behind the loading screen (`renderer.warmUp()` after `bindWorld` — M3-02d: one throwaway off-screen frame with every GL program and every pooled sprite in it, never presented) and clears held input on `blur` (M1-17); since M3-01 the replay library loaded before the title (`Shell.replays`, `core/replay` `createReplayLibrary`), `ShellOptions.shareReplay` / `buildId` for the scene flow, and rumble wired when the adapter has `rumble` |
...
| `dispatch` | implemented | Sim event → presentation handler routing, allocation-free; `connectFxEvents` feeds the renderer's particles, shake / flash / dim and score popups from the World's events (M1-14); `connectAudioEvents` feeds `Sfx` / `Music` / `MusicDuck` to the audio engine (M1-15); `connectOptionEvents` turns the Options screen's `UserOption` events into bus volumes (`volumeGain`; SFX drives `sfx` and `ui`) and profile switches, `applyAudioOptions` sets the saved volumes at boot (M1-17); since M2-02 also the bullet palette event → a callback (the renderer's `setBulletPalette`); since M2-08 the SCALE / SHAKE / FLASHES / HITBOX events → a `DisplayTarget` (the renderer) and `applyDisplayOptions` for the saved ones; M2-09's BOSS HP event is ignored — the core HUD reads the saved option; since M2-10 `connectStagePreparation` answers `PrepareStage` — the audio engine prepares that stage's music set plus the title theme (`StagePreparationTarget`); since M2-15 `connectSoundTest` answers `SoundTest` — the audio engine loads and plays a music-library track by index (`SoundTestTarget`); since M2-16 `connectOptionEvents`' sixth argument answers `UserOptionKind.InputSettings`; `connectRumbleEvents` — the core's `Rumble` events to the adapter's gamepad rumble while the save's RUMBLE is on (M3-01); since M3-02 the `CrtFilter` and `Aspect` events (and `applyDisplayOptions`) reach the same `DisplayTarget` through its optional `setCrtFilter` / `setAspect` |
| `controls` | implemented | M2-16: `createShellControls({ save, input, profiles })` — the core scene flow's `ControlsSetup` (the Options screen's REBIND KEYS / PAD and the rebind screen): the devices (the app's `rebindable()` profiles, read on every call), key labels, the adapter's capture (`WebInput.beginCapture`), `bindCaptured` (`captureToken` + `rebindAction` with conflict detection), `reset` (`resetBindings`); every change stored in the save's `options.input.bindings` and applied through the app's `customize` — [`docs/dev/options-rebinding-and-accessibility.md`](../../docs/dev/options-rebinding-and-accessibility.md#the-rebind-screen) |
| `error-screen` | implemented | Boot overlay: progress bar and error screen (Canvas 2D) |
| `frame-loop` | implemented | `requestAnimationFrame` driver (moved here from the apps); the refresh-rate probe `createRefreshMonitor` (M2-08: the interquartile mean of the last 31 rAF deltas — `INTERPOLATION_MIN_HZ` 70 decides render interpolation; since M3-02b `VSYNC_LOCK_MIN_HZ` 55 … `VSYNC_LOCK_MAX_HZ` 65 decide the loop's vsync lock, one tick per frame on a 60 Hz panel — `ShellOptions.framePacing`) |
...
| `debug` | implemented | The dev / test builds' debug tools (M1-19; not named in the plan — keys and timing are host work): `debugToolsFactory` / `createDebugTools` (core `createDebugControls` + render-pixi `createDebugOverlay`), `DEBUG_KEYS` (F1–F8, and 1–8 on the TV), the TV unlock `DEBUG_UNLOCK_SEQUENCE` (Pause, Ch+ ×3 within 3 s), per-frame timing hooks, `window.__shmupDebug` (`ShmupDebugApi`; its `renderer` since M2-08); M2-17: `window.__shmupDebug.save` — export / import the save, the storage usage (`DebugSaveApi`) — and the overlay's device line (`DebugToolsOptions.device`, the TV's model and firmware); M3-02f: `DebugToolsOptions.reportUrl` — with a log-server URL the frame hooks also feed the `telemetry` module's sampler (raw frame / tick / render times, draw calls, structure rebuilds, pooled render targets) and `DebugTools.telemetry` exposes the capture |
| `telemetry` | implemented | M3-02f: the guided render-profile capture streamed to the input probe's log server (`npm run log-server`; built with `VITE_REPORT_URL` → `__SHMUP_REPORT_URL__`) — `createRenderTelemetry`, the allocation-free `RenderSampler` (an inbox `Float64Array` per frame; a 3-second window closes into min / median / p95 / max of the frame, tick, render and draw-call series, a quantized histogram of each of them so the analyzer can pool a group's frames into a **true** p95 rather than a median of p95s, the TPF and rAF buckets, and counts the frames a POST was in flight so a sender-induced spike can be excluded), the sticky `RenderChecklist` (M1–M8 of the review's §4 table, drawn as a dev-only `<div>` updated once per window) and the `XMLHttpRequest` `RenderReporter` / `RenderSampleQueue`. Absent from a release bundle (`apps/tizen/test/build/tizen-build.test.ts`, `apps/web/test/build/web-build.test.ts`, and `test/integration/render-telemetry-pipeline.test.ts` for the source-level gate) |
| `storage` | implemented | M2-17: `createWebStorage` — the web / TV hosts' one `localStorage` adapter with quota checks (the app's 1 MiB budget, per-value limit, `QuotaExceededError` → drop `save.corrupt` and retry once, else memory for that value; any other error → memory for the session; `usage()`, `issues`); the debug save export / import `exportSaveText` / `importSaveText`; the replay library's keys (`replay.last`, `replay.1`–`3`, M3-01) are sized to fit beside the save |
| `memory` | implemented | M2-17: the TV memory budget (< 100 MB, shmup_tech.md §2.5): `estimateMemory` / `estimateStageMemory` (atlas pages + decoded images, SFX bank, the zone's music set, render targets, heap baseline — every campaign zone is tested under budget; M3-02d corrected the render-target term — `potBytes(w, h)` for Pixi's power-of-two pooling, the exact frame target, the exported `FILTER_TARGETS` nesting depth, and the `crtFilter` / `crtAsFilter` inputs: the shipped CRT costs no target at all, and the deliberately conservative legacy term is only for `screenPass: 'filter'`), the zones' sprite sets and atlas-page needs (`stageSpriteSets`, `atlasPageNeeds`, `stagePages`) and `createAtlasResidency` / `connectAtlasResidency` — pages the next zone does not need leave the GPU on `PrepareStage` (`Shell.atlasResidency`; nothing with today's single page) |
| `determinism` | implemented | M2-18: the cross-engine determinism check — `createDeterminismCheck(contentFiles, now?)` validates the content like the boot and `play(replay)` plays a `core/replay` document headless, returning the state hashes the page's engine computed (`DeterminismRun`); `installDeterminismCheck` publishes it as `window.__shmupDeterminism` and marks `<html data-shmup-determinism>` — the web app's `?determinism` page in dev / test builds, played by `test/e2e/determinism.spec.ts` in Chromium and Firefox |
