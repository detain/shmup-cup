 ...
 * start with `ALLOCATION_GUARD_EXEC_ARGV` so per-frame code (e.g. the free-flight scene's
 * `update()`) can be checked with the core's allocation guard
 * (`packages/core/test/helpers/alloc.ts`). Run with
...
export default defineShmupProject('shell', { execArgv: ALLOCATION_GUARD_EXEC_ARGV });
