...
code, firmware, display, Chrome and WebGL (`device-info`) — as the panel's last line, logging the
snapshot as `Shmup Cup device`. Since M3-02c the panel's seventh line carries the render profile
(`REB` / `RT`) and a dev / test build reads `WEBGL_VERSION_KEY` (`localStorage['shmup-cup:gl']`,
`'2'` → a WebGL2 context) so the owner can A/B the renderer on the monitors — the review's **F8**;
**WebGL1 stays the shipped default** and a release bundle never reads the key. Package and install a debug build
